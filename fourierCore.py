...
def distressCheckFourierV2(equalized, weightedProfile, threshScore, tolerant=True):
...
def makeThreshV2(filename):
    ...
    # threshScore = weightedPower(fourierFilter(distress), fourierFilter(weightedProf)) * 0.055
...
def fourierCalibrationV9(t, Hz, eogChan):
    # acts as a main, invokes the pullFourierProfiles() method to collect data once for neutral and once for distress.
    # uses this to generate a threshold score related to the power generated by a given fourier transform.
    query = input("please input 'y' if you want to display plots, enter if not.\n")
    if query == 'y':
    ...
    if query == 'y':
        ...
        speech = False
    query = input("please input 'y' if you want vibrational feedback, enter if else.\n")
    if query == 'y':
        vibration = True
    else:
        vibration = False
